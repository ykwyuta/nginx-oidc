...
http {
    ...
    server {
        ...
        location / {
            ...
            # 任意クレーム変数（継続アクセスでもセッションCookieから復元）:
...
| `$oidc_claim_<name>` | JWT / UserInfo の任意クレーム（例: `$oidc_claim_groups`, `$oidc_claim_roles`, `$oidc_claim_tenant_id`） |
...
**継続認証（`oidc_auth` Cookie による再訪問）** では、セッション Cookie のペイロードに `B64(sub):B64(email):B64(name):timestamp[|B64(key):B64(val)...]` を含めているため、`$oidc_claim_sub` / `$oidc_claim_email` / `$oidc_claim_name` の 3 変数に加え、**任意クレームもすべて** セッション Cookie から復元されます（クッキーサイズ上限: 3500 バイト）。

**任意クレーム変数（`$oidc_claim_<name>`）** は初回認証リクエスト時に JWT クレームまたは UserInfo レスポンスから取得され、セッション Cookie に格納されるため、継続アクセスリクエストでも `$oidc_claim_groups` 等が利用可能です。
...
| 7 | `proxy_pass $arg_url` による SSRF リスク | `$oidc_discovery_url` / `$oidc_jwks_url` 変数を追加。nginx.conf でこれらの変数を `proxy_pass` に使用することで URL の出所を設定値に限定 |
...
| 12 | `oidc_return_to` Cookie にクエリ文字列が含まれない | `r->uri + '?' + r->args` を連結して保存。4096 バイト制限内に収める |
...
| `$oidc_access_token` 変数 | ✅ 実装済み。`proxy_set_header Authorization "Bearer $oidc_access_token"` で利用可能 |
...
| ~~UserInfo エンドポイント~~ | ✅ 実装済み（`oidc_use_userinfo on` で有効化） |
...
| **SSRF 対策強化**（問題 7） | ✅ 完了。`$oidc_discovery_url` / `$oidc_jwks_url` 変数を追加。nginx.conf での使用例を更新 |
...
| **`$oidc_access_token` 変数** | ✅ 完了。Bearer トークン転送が可能 |
...
| 項目 | 説明 |
|------|------|
| **UserInfo エンドポイント対応** | ✅ 実装済み。`oidc_use_userinfo on` で有効化。Google/Microsoft 等の最小クレーム IdP に対応 |
| **継続リクエストでの任意クレーム** | ✅ 実装済み。セッション Cookie に extra_claims を格納・復元。継続アクセスでも `$oidc_claim_groups` 等が利用可能 |
| **RP-Initiated Logout** | シングルサインアウトの実現に必要。本番アプリではほぼ必須 |
| **SSL/TLS 設定 (`oidc_ssl_trusted_certificate`)** | 自己署名証明書・内部 CA 環境での信頼性確保 |
| **任意クレームの変数展開 (`$oidc_claim_<name>`)** | ✅ 実装済み。`$oidc_claim_groups`, `$oidc_claim_roles`, `$oidc_claim_tenant_id` など任意の JWT クレームに対応 |
...
✅ P2 (部分): 任意クレーム変数 ($oidc_claim_*)  ← 完了
✅ P2 (追加): UserInfo エンドポイント対応 / 継続リクエストでの任意クレーム永続化  ← 完了
  ↓
...
P0+P1+P2（UserInfo・任意クレーム永続化含む）が完了しました。実際の IdP との接続・本番投入に必要な機能が揃っています。
...
**P0/P1 の全課題が解決済み**：
- ✅ **PKCE 実装**（P0）: S256 メソッドで実装。`oidc_pkce_verifier` Cookie に verifier を保持
- ✅ **Cookie ペイロードのデリミタ問題**（P1）: `ngx_encode_base64()` で各クレームをエンコード
- ✅ **SSRF 対策**（P0）: `$oidc_discovery_url` / `$oidc_jwks_url` 変数を追加。nginx.conf の設定例を更新
- ✅ **`access_token` オプション化**（P1）: `id_token` のみ必須に変更
- ✅ **`oidc_scope` ディレクティブ**（P1）: デフォルト `"openid"`、任意スコープを設定可能
- ✅ **`$oidc_access_token` 変数**（P1）: Bearer トークン転送に対応
- ✅ **`oidc_return_to` クエリ文字列**（P1）: 認証後に元のクエリ文字列も復元

**P2 任意クレーム変数**（前回実装済み）：
- `$oidc_claim_<name>` プレフィックス変数を実装。`$oidc_claim_groups`, `$oidc_claim_roles`, `$oidc_claim_tenant_id` など任意の JWT クレームを NGINX 変数として利用可能
- `NGX_HTTP_VAR_PREFIX` フラグで単一ハンドラが全 `$oidc_claim_*` 変数を処理

**P2 UserInfo エンドポイント対応 / 継続リクエストでの任意クレーム永続化**（今回実装）：
- `oidc_use_userinfo on` ディレクティブを追加。JWT 検証後に UserInfo エンドポイントを呼び出し、返却されたクレームを `extra_claims` にマージ
- `$oidc_userinfo_url` 変数を追加。SSRF 対策として `proxy_pass $oidc_userinfo_url` で使用
- セッション Cookie のペイロードを `B64(sub):B64(email):B64(name):timestamp[|B64(key):B64(val)...]` に拡張。任意クレームも Cookie に格納・復元
- 継続アクセスリクエストでも `$oidc_claim_groups` 等の任意クレーム変数が利用可能（クッキーサイズ上限 3500 バイト内で格納）
- UserInfo 失敗時はフォールバックとして ID トークンのクレームのみで Cookie を発行（非致命的エラー）
- `ngx_http_oidc_issue_session_and_redirect` 関数に Cookie 発行ロジックを集約し、jwks_handler・userinfo_handler の両方から呼び出せるよう再設計

**残存する課題**（P2 後半以降）：
- RP-Initiated Logout、SSL/TLS 設定 など

P0+P1+P2（UserInfo・任意クレーム永続化含む）が完了したことで、Google・Microsoft Azure AD など最小クレーム IdP も含めた本番環境への投入が実現可能なレベルに達しています。
