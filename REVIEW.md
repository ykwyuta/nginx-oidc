...
| `$oidc_claim_<name>` | JWT の任意クレーム（例: `$oidc_claim_groups`, `$oidc_claim_roles`, `$oidc_claim_tenant_id`） |
...
**継続認証（`oidc_auth` Cookie による再訪問）** では、セッション Cookie のペイロードに `sub:email:name:timestamp` を含めているため、`$oidc_claim_sub` / `$oidc_claim_email` / `$oidc_claim_name` の 3 変数は常に正しく復元されます。

**任意クレーム変数（`$oidc_claim_<name>`）** は初回 JWT 検証リクエスト時のみ利用可能です。セッション Cookie には sub/email/name のみが格納されるため、`groups` 等の追加クレームは継続アクセスリクエストでは利用できません。これらが常に必要な場合は UserInfo エンドポイント対応（P2 以降）を検討してください。
...
✅ P2 (部分): 任意クレーム変数 ($oidc_claim_*)  ← 完了
  ↓
P2 (残): UserInfo → RP-Initiated Logout → SSL設定
  ↓
...
P0+P1 が完了し、任意クレーム変数も実装済みです。実際の IdP との接続に必要な実用レベルに到達しています。
...
- 初回 JWT 検証時に全クレームを `extra_claims` 配列に格納し、`$oidc_claim_sub/email/name` はセッション Cookie から継続アクセスでも復元

**残存する課題**（P2 後半以降）：
- UserInfo エンドポイント対応、RP-Initiated Logout、SSL/TLS 設定など

P0+P1+P2（任意クレーム変数）が完了したことで、本番環境への投入が現実的なレベルに到達しています。
